...
static void treesit_walk_tree_1 (Lisp_Object node, Lisp_Object fn,
                                           Lisp_Object parent_nodes);

DEFUN ("treesit-walk-tree", Ftreesit_walk_tree,
       Streesit_walk_tree, 2, 3, 0,
...
Usage: (treesit-walk-tree NODE FN &optional PARENT-NODES)  */)
  (Lisp_Object node, Lisp_Object fn, Lisp_Object parent_nodes)
{
  if (NILP (node)) return Qnil;
  CHECK_TS_NODE (node);
  treesit_walk_tree_1 (node, fn, parent_nodes);
  return Qnil;
}

/* Helper function - optimized for performance */
static void
treesit_walk_tree_1 (Lisp_Object node, Lisp_Object fn,
                               Lisp_Object parent_nodes)
{
  ...
  for (uint32_t i = 0; i < child_count; i++)
    {
      ...
      /* Recursive call */
      treesit_walk_tree_1 (child_node, fn, new_parent_nodes);
    }
}
